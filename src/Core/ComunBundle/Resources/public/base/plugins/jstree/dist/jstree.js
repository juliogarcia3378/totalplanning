            ...
            if(this.settings.checkbox.three_state) {
                this.element
                    ...
                    .on('deselect_node.jstree', $.proxy(function (e, data) {
                        ...
                        var tmp2=[];
                        for(i = 0, j = obj.children_d.length; i < j; i++) {
                            if(this._model.data[obj.children_d[i]].state.disabled == false)
                                this._model.data[obj.children_d[i]].state.selected = false;
                            else
                                tmp2.push(obj.children_d[i]);
                        }
                        for(i = 0, j = obj.parents.length; i < j; i++) {
                        ...
                        if(dom.length) {
                            dom.find('.jstree-anchor:not(.jstree-disabled)').removeClass('jstree-clicked');
                        }
                    }, this))
